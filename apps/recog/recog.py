...
class LetterSegmenter:
    ...
    def segment_letters(
        ...
        sorted_contours = sorted(
            merged_contours, key=lambda c: cv2.boundingRect(c)[0])
        ...
        for contour in sorted_contours:
            ...
            # Dynamic padding based on letter size
            pad = max(5, min(w, h) // 4)
            x1 = max(0, x - pad)
            y1 = max(0, y - pad)
...
class LetterRecognizer:
    ...
    def _load_model(self, model_path: str) -> torch.nn.Module:
        ...
        try:
            # 36 classes for model compatibility
            model = KyrgyzCNN(len(self.classes))
    ...
    def recognize_letter(
        ...
        pil_image = Image.fromarray(
            cv2.cvtColor(letter_image, cv2.COLOR_BGR2RGB))
        ...
        with torch.no_grad():
            ...
            logger.debug(
                f"Base prediction: {predicted_upper} (conf: {confidence:.3f})")
            ...
            for i in range(topk):
                ...
                logger.debug(
                    f"Top-{i+1}: {self.classes[idx]} (conf: {conf:.3f})")

        return RecognitionResult(
            letter=predicted_letter,
            # Adjust confidence based on case certainty
            confidence=confidence * (0.5 + 0.5 * abs(case_score - 0.5)),
            bounding_box=bbox,
            image=letter_image
        )
...
def process_folder_with_evaluation():
    ...
    try:
        ...
        logger.info(
            f"Average word accuracy: {metrics['avg_word_accuracy']:.2f}%")
...
def recog(image_path):
    ...
    model_path = "apps/recog/best_model.pth"
